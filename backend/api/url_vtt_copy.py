...
class URL_VTT():
    ...
    def process_lesson(self):
        ...
        if self.videoFormat:
            media_name = f"lessons/{self.uuid}/{self.uuid}.mp4"
        else:
            media_name = f"lessons/{self.uuid}/{self.uuid}.mp3"

        print("BEFORE ASSIGNMENT:", repr(self.lesson.media_file.name))

        self.lesson.media_file.name = media_name

        print("AFTER ASSIGNMENT:", repr(self.lesson.media_file.name))
        print("MEDIA FOLDER:", repr(self.lesson.media_folder))
        ...
        print("AFTER DATABASE SAVE:", repr(self.lesson.media_file.name))
        print("AFTER DATABASE FOLDER:", repr(self.lesson.media_folder))
        print("Done!")
